...
class Runner:
    ...
    def run(self, n_steps, blueprint, ppo, transition_buffer_dict=None):  # TODO add transition_buffer + episode):
        ...
        while cur_timestep < self.env.timesteps:
            ...
            if done or (cur_timestep == self.env.timesteps - 1):
                ...
                if not transition_buffer_dict:
                    ...
                    transition_buffer_dict = {}
                elif len(transition_buffer_dict) == 0:
                    transition_buffer = my_transition_buffer
                    transition_buffer_dict = {}
                else:
...
class MultiRunner:
    ...
    def fork(self, target, common_args=None, specific_args=None):
        ...
        process1 = multiprocessing.Process(target=target, args=(self.runners[0], *common_args, *[specific_args[0][0]]))
        process2 = multiprocessing.Process(target=target, args=(self.runners[1], *common_args, *[specific_args[0][1]]))
        process3 = multiprocessing.Process(target=target, args=(self.runners[1], *common_args, *[specific_args[0][2]]))
        process4 = multiprocessing.Process(target=target, args=(self.runners[1], *common_args, *[specific_args[0][3]]))
    ...
    def run(self, n_steps, blueprint, ppo):
        ...
        self.fork(target=Runner.run, common_args=(n_steps, blueprint, ppo), specific_args=(transition_buffer_dicts,))
...
class MindmapPPO:
    ...
    def __init__(self, param_file="src/model_params_mt.yaml", quiet=False):
        ...
        print("Spawned new Mindmap PPO instance!")
        ...
        self.buffer = MindmapRolloutBuffer(self.env.num_states_iri, self.env.num_actions,
                                           ...
                                           self.env.timesteps, self.env.num_objectives, self.gamma, self.lam, self.processes)
    ...
    def run_episodes(self, exec_mode="train", checkpoint_dir=None, checkpoint_ep=None):
        ...
        if (exec_mode == "train") or (exec_mode == "continue_training"):
            ...
            for episode in range(self.n_epochs):
                ...
                transition_buffers_list = self.runner.run(self.env.timesteps, self.buffer, self)
    ...
    def train(self, buffer=None):
        ...
        (observation_buffer_init,
         ...
         return_buffer_init,
         logprobability_buffer_init) = buffer.get()
    ...
    def _normalize_advantages(self, buffer=None):
        if not buffer:
            buffer = self.buffer

        buffer.counter, buffer.trajectory_start_index = 0, 0
        advantage_mean, advantage_std = (np.mean(buffer.advantage_buffer),
                                         np.std(buffer.advantage_buffer))

        buffer.advantage_buffer = (buffer.advantage_buffer - advantage_mean) / advantage_std
