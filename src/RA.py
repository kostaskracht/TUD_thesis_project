...
class RA:
    ...
    def __init__(
        ...
        self.output_dir = f"outputs/ra/{self.timestamp}"
...
def execute_RA(model_file=None, env_file=None, reuse_mode="no", epsilon=0.0001, continue_execution=False, file_to_load=None):
    ...
    writer = SummaryWriter(f"runs/ra/{ra.timestamp}")
    ...
    if continue_execution:
        ...
        ra.m = input_dict["m"]
        ra.W = input_dict["W"]
        ra.ccs = input_dict["ccs"]
        ra.ccs_weights = input_dict["ccs_weights"]
        ra.queue = input_dict["queue"]
        ra.iteration = input_dict["iteration"]
    ...
    while not ra.ended():
        ...
        logging.info(f"==============================================================================")
        logging.info(f"Beginning RA iteration {ra.iteration}")

        value, prev_runs_metadata = solve(w, ra.output_dir, prev_runs_metadata, reuse_mode, MODEL_FILE=model_file, ENV_FILE=env_file)
        logging.info(f"Value from PPO execution: {value}")
        ra.add_solution(value, w)
        # Plot the convex coverage set
        ra.plot_ccs(ra.ccs, ra.ccs_weights, writer=writer)

        # Save RA iteration parameters
        output_dict = {"m": ra.m,
                       "W": ra.W,
                       "ccs": ra.ccs,
                       "ccs_weights": ra.ccs_weights,
                       "queue": ra.queue,
                       "iteration": ra.iteration
                       }
        with open(f'{ra.output_dir}/iters/iter_{ra.iteration}.json', "w") as f:
            json.dump(output_dict, f, cls=NumpyEncoder)


        hv = hypervolume(np.zeros(m), ra.ccs)
        sp = sparsity(ra.ccs)
        tm = time.time() - start
        ...
        writer.add_scalars("RA Results",
                           {"Hypervolume": hv,
                            "Sparsity": sp,
                            "Time (sec)": tm},
                           ra.iteration)

    logging.info(f"RA execution {ra.timestamp} finished.")
    logging.info(f"==============================================================================")
...
if __name__ == "__main__":
    ...
    ra = execute_RA(model_file=model_file, env_file=env_file, reuse_mode=reuse_mode, epsilon=epsilon)
